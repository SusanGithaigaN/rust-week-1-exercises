...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951350;
...
pub fn is_valid_tx_fee(fee: f64) -> bool {
    ...
    let result = fee >= 0.00001 && fee <= 0.01;
    println!("The Fee value is {} BTC and is valid: {}", fee, result);
    result
}
...
pub fn normalize_address(address: &str) -> String {
    ...
    let trimmed_addr = address.trim();
    let lowercase_addr = trimmed_addr.to_lowercase();
    lowercase_addr
}
...
pub fn unpack_wallet_info(wallet_info: (String, f64)) -> String {
    ...
    let (wallet_name, wallet_bal) = wallet_info;
    format!("Wallet {} has balance: {:.0} BTC", wallet_name, wallet_bal)
}

/// Convert BTC to satoshis (1 BTC = 100,000,000 sats).
pub fn calculate_sats(btc: f64) -> u64 {
    // TODO: Multiply btc by BTC_TO_SATS and return as u64
    let sats = btc * BTC_TO_SATS as f64;
    sats as u64
}
...
pub fn generate_address(prefix: &str) -> String {
    ...
    let mut i = 0;
    while i < suffix_len {
        let random_char = valid_chars[random_gen.gen_range(0..valid_chars.len())];
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        return (false, String::from("Block height cannot be negative"));
    }
    if height > 800_000 {
        return (false, String::from("Block height is unrealistic"));
    }
...
pub fn halving_schedule(blocks: &[u64]) -> HashMap<u64, u64> {
    ...
    while i < blocks.len() {
        let block = blocks[i];
        let btc_halvings = block / 210_000;
        let block_reward = base_reward >> btc_halvings;
        result.insert(block, block_reward);
        i += 1;
    }
    result
}
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    let tx_version = u32::from_le_bytes(version_bytes);
    Ok(tx_version)
}
